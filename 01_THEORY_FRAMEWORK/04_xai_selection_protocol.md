...
The selection of techniques is not arbitrary or convenience-based; it follows a **methodological logic** aligned with the project’s conceptual model:
...
The objective is to ensure that the *XAI-TrustFramework* includes representative techniques that:
...
2. **Cover complementary explanation paradigms** — ensuring that both *attributive* (why something happened) and *counterfactual* (what could be changed) reasoning are represented.  
...
4. **Are reproducible, interpretable, and widely adopted** — techniques with stable, open-source implementations that allow for transparent replication and comparison.
...
Following this logic, the selected set of techniques satisfies the following strategic goals:
...
- **Cross-domain scalability** — ensure the framework can be generalized to regression, classification, and image domains.

The chosen techniques therefore provide a **complementary and traceable foundation** for testing the multidimensional model of trust in XAI systems.
...
| **Anchors (Ribeiro et al., 2018)** | Post-hoc, model-agnostic | Uses rule-based local approximation | Local | Cognitive (local fidelity, precision, coverage) |
...
|-----------|----------------|
| **Data compatibility** | Designed for tabular, structured data; TreeSHAP is optimized for ensembles like RandomForest and XGBoost. |
| **Task type** | Regression tasks with continuous outcomes. |
| **Explainability scope** | Offers both local (instance-level) and global (dataset-level) explanations. |
...
SHAP enables the **empirical validation of its own theoretical guarantees**—linking *local accuracy* and *consistency* to measurable trust notions such as *fidelity* and *stability*.  
...
### 4.1 Conceptual complementarity
While SHAP explains *why* a model made a certain prediction, DiCE (Mothilal et al., 2020) explains *what would need to change* to obtain a different outcome.  
This introduces **actionability** and **contestability** into the trust framework—two notions central to human-centred and responsible AI.
...
| Criterion | Justification |
|-----------|----------------|
| **Model dependency** | Model-agnostic: requires only access to inputs and predictions. |
| **Complementary perspective** | Captures “what-if” scenarios instead of additive decomposition. |
| **Trust dimension** | Operationalizes the *social dimension* of trust (actionability, feasibility, diversity). |
| **Metrics supported** | Actionability, Plausibility, Diversity. |
| **Compatibility** | Can be applied to the same models used with SHAP, enabling cross-validation. |
...
It formalizes the idea of an “anchor” — a minimal set of feature conditions that, if satisfied, guarantee the same model prediction with high probability.

Its key property is **precision under locality**, ensuring that within the anchor’s region, predictions remain consistent.

### 5.2 Methodological alignment
| Criterion | Justification |
|-----------|----------------|
| **Data compatibility** | Tabular data with categorical or structured variables. |
| **Task type** | Classification tasks. |
| **Explainability scope** | Local and discrete; produces human-readable logical rules. |
...
| **Empirical precedent** | Used widely in fairness auditing and human evaluation studies. |
...
While it lacks formal axioms like SHAP, it provides **spatial faithfulness guarantees** through gradient-based consistency.
...
| **Empirical precedent** | Extensively validated in medical imaging and industrial inspection tasks. |

### 6.3 Expected contribution
Grad-CAM extends the framework to **perceptual trust**, testing whether explanations visually correspond to meaningful areas for humans.  
...
| **CEM / ProtoDash / ConceptSHAP** | Tailored for image or concept-based reasoning, outside the scope of tabular and baseline experiments. |
...
The combination of SHAP and DiCE enables the framework to:
- Cover **two complementary paradigms** (attributive and counterfactual).  
- Operationalize **two major trust dimensions** (technical and social).  
- Provide a **controlled comparison** between model-specific and model-agnostic approaches.  
- Support **reproducible trust metrics** across user roles and dimensions.

Anchors and Grad-CAM further demonstrate **cross-domain generalization**, adding symbolic and perceptual interpretability layers.

Thus, the selection captures both **internal consistency** (reliability of explanations) and **external usability** (human interpretability and actionability).
...
- **TS4NLE or similar systems** for human-centered evaluation of comprehensibility and narrative quality.

Each new technique will be assessed through the same logic of alignment:
> *Does it enable the measurement of trust notions within one or more framework dimensions under realistic conditions?*
